...
public class SettingFragment extends BaseFragment {
    ...
    private void initView(View rootView) {
        ...
        mChoiseVideo.setOnClickListener(v -> openDocumentedResult.launch("video/*"));
        ...
        mProtectMethodBtn.setOnClickListener(view -> {
            ...
            mPopupMenu.setOnMenuItemClickListener(item -> {
                if (item.getItemId() == R.id.protect_method_disable_camera) {
                    onMethodTypeClick(MethodType.TYPE_DISABLE_CAMERA);
                } else if (item.getItemId() == R.id.protect_method_local) {
                    onMethodTypeClick(MethodType.TYPE_LOCAL_VIDEO);
                } else if (item.getItemId() == R.id.protect_method_network) {
        ...
        if (method_type > 0) onMethodTypeClick(method_type);
    }
    ...
    private void onMethodTypeClick(int type) {
        ...
        switch (type) {
            ...
            case MethodType.TYPE_LOCAL_VIDEO:
                ...
                mInput.setText(mHasOpenDocuments ? MultiPreferences.getInstance().getString("video_path_local", "") : "");
            ...
            case MethodType.TYPE_LOCAL_VIDEO:
                ...
                new Thread(() -> {
                    ...
                    HandlerUtil.runOnMain(() -> {
                        try { progressDialog.dismiss(); } catch (Throwable e) {}
                    });
                }).start();
                break;
            case MethodType.TYPE_NETWORK_VIDEO:
    ...
    private boolean copyLocalVideo(String u) {
        ...
        try {
            ...
            if (!TextUtils.isEmpty(video_path_local)) new File(video_path_local).delete();
            String subfix = u.substring(u.lastIndexOf("."));
